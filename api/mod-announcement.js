export default async function handler(req,res){
  ...
  try{
    ...
    const log=await fetch(`${url}/rest/v1/club_event_hub_log`,{
      method:"POST",
      headers:{...headers,Prefer:"return=minimal"},
      body:JSON.stringify({
        event_type:"mod_announcement",
        title,
        payload:{message:`🛡️ ${title}\n${text}`,moderatorId:user.id}
      })
    });
    ...
    if(webhook){
      const r=await fetch(webhook,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({content:`🛡️ **${title}**\n${text}`,allowed_mentions:{parse:[]}})});
      discordSent=r.ok;
    }
