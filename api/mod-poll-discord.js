export default async function handler(req,res){
  ...
  try{
    ...
    const options=Array.isArray(body.options)
      ? body.options.map(v=>String(v||"").trim()).filter(Boolean).slice(0,8)
      : [];
    ...
    const discordRes=await fetch(webhook,{
      ...
      body:JSON.stringify({
        ...
        username:"ACY Club",
        avatar_url:`${process.env.PUBLIC_SITE_URL||""}/icons/acy-192.png`
      })
    });
    if(!discordRes.ok){
      return res.status(502).json({error:`Discord Webhook ${discordRes.status}: ${await discordRes.text()}`});
    }

    // Optional log in the existing event hub log.
    await fetch(`${url}/rest/v1/club_event_hub_log`,{
      method:"POST",
      headers:{...headers,Prefer:"return=minimal"},
      body:JSON.stringify({
        event_type:"mod_poll",
        title:`🗳️ ${question}`,
        payload:{message:content,moderatorId:user.id,options}
      })
    }).catch(()=>{});

    return res.status(200).json({ok:true,discordSent:true});
  }catch(error){
    console.error("mod-poll-discord",error);
    return res.status(500).json({error:error?.message||"Discord-Vote-Benachrichtigung fehlgeschlagen."});
  }
}
