export default async function handler(req,res){
  ...
  try{
    ...
    const logRes=await fetch(`${url}/rest/v1/club_event_hub_log`,{
      method:"POST",
      headers:{apikey:serviceKey,Authorization:`Bearer ${serviceKey}`,"Content-Type":"application/json",Prefer:"return=representation"},
      body:JSON.stringify({event_type:eventType,title,payload})
    });
    if(!logRes.ok){
      const txt=await logRes.text();
      throw new Error(`Event log: ${txt}`);
    }
    ...
    if(discordWebhook){
      try{
        const r=await fetch(discordWebhook,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({content:message,allowed_mentions:{parse:[]}})});
        discordSent=r.ok;
      }catch{}
    }

    // Twitch Send Chat Message API
    ...
    if(twitchToken&&broadcasterId&&botUserId&&clientId){
      try{
        const r=await fetch("https://api.twitch.tv/helix/chat/messages",{
          method:"POST",
          headers:{Authorization:`Bearer ${twitchToken}`,"Client-Id":clientId,"Content-Type":"application/json"},
          body:JSON.stringify({broadcaster_id:broadcasterId,sender_id:botUserId,message})
        });
        if(r.ok){
          const out=await r.json().catch(()=>({}));
          twitchSent=Boolean(out?.data?.[0]?.is_sent);
        }
      }catch{}
    }

    await fetch(`${url}/rest/v1/club_event_hub_log?event_type=eq.${encodeURIComponent(eventType)}&order=created_at.desc&limit=1`,{
      method:"PATCH",
      headers:{apikey:serviceKey,Authorization:`Bearer ${serviceKey}`,"Content-Type":"application/json"},
      body:JSON.stringify({twitch_sent:twitchSent,discord_sent:discordSent})
    }).catch(()=>{});

    return res.status(200).json({ok:true,eventType,twitchSent,discordSent});
  }catch(error){
    console.error("club-event-hub",error);
    return res.status(500).json({error:error?.message||"Event Hub failed."});
  }
}
