export default async function handler(req,res){
  ...
  if(!url||!key)return res.status(503).json({error:'Moderationsdienst nicht konfiguriert.'});
  ...
  try{
    ...
    const isAdmin=ar.ok&&(await ar.json()).length>0; const isMod=mr.ok&&(await mr.json()).length>0;
    if(!isAdmin&&!isMod)return res.status(403).json({error:'Nur Moderatoren.'});
    if(req.method==='GET'){
    ...
    if(req.method==='PATCH'){
      ...
      if(action==='mute24' && report.target_user_id){
        const mute=await fetch(`${url}/rest/v1/club_chat_bans`,{method:'POST',headers:{...headers,Prefer:'resolution=merge-duplicates'},body:JSON.stringify({user_id:report.target_user_id,banned_until:new Date(Date.now()+24*3600*1000).toISOString(),reason:report.reason,created_by:user.id})});
      ...
      const patch=await fetch(`${url}/rest/v1/club_reports?id=eq.${encodeURIComponent(id)}`,{method:'PATCH',headers:{...headers,Prefer:'return=minimal'},body:JSON.stringify({status,handled_at:new Date().toISOString(),handled_by:user.id,moderator_note:String(body.note||'').slice(0,500)})});
