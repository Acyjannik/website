...
function berlinDayStartUtc(date = new Date()) {
  ...
  const offsetHour = Number(new Intl.DateTimeFormat('de-DE', { timeZone: 'Europe/Berlin', hour: '2-digit', hour12: false }).format(utcMidnight));
...
async function sendDailyStreakReadyNotifications({ base, key }) {
  const readyBefore = new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString();
  ...
  for (const row of streaks || []) {
    ...
    const body = ageHours <= 48 ? `Deine 24 Stunden sind vorbei. Hol dir jetzt deinen Check-in und halte deine ${Number(row.current_streak || 0)}-Tage-Serie am Leben. 💜` : 'Dein Check-in ist wieder verfügbar. Wenn du die 48-Stunden-Gnadenfrist verpasst hast, startest du damit eine neue Serie. 💜';
...
const GREETINGS = {
  morning: { title: '🌅 Guten Morgen, ACY Club!', body: 'Starte entspannt in den Tag. Denk kurz an dein Tier, schau bei deinen Quests vorbei und hol dir deine nächsten XP. 💜', link: '/club-profile.html#pet-section', tag: 'acy-morning' },
  evening: { title: '🌙 Schönen Abend im ACY Club!', body: 'Zeit für einen kleinen Check-in: Dein Tier, deine Quests und deine Community warten auf dich. 💜', link: '/club-profile.html#club-quests-section', tag: 'acy-evening' }
};
...
async function sendGreeting({ mode, base, key, allowDuplicate = false }) {
  ...
  for (const profile of profiles || []) {
    ...
    if (inAppEnabled) notificationRows.push({ user_id: profile.id, title: greeting.title, body: greeting.body, notification_type: notificationType, link_url: greeting.link });
    if (pushEnabled) {
      const result = await sendPushToUser({ supabaseUrl: base, serviceKey: key, userId: profile.id, title: greeting.title, body: greeting.body, url: greeting.link, tag: greeting.tag });
      sentPush += Number(result.sent || 0); removed += Number(result.removed || 0); failed += Number(result.failed || 0);
    }
  }
  if (notificationRows.length) {
    const notificationRes = await sb(base, key, '/rest/v1/club_notifications', { method: 'POST', headers: { Prefer: 'return=minimal' }, body: JSON.stringify(notificationRows) });
    if (notificationRes.ok) sentInApp += notificationRows.length; else failed += notificationRows.length;
  }
...
export default async function handler(req, res) {
  ...
  if (isCron) {
    ...
    try {
      ...
      if (minute !== 0 || ![8, 18].includes(hour)) return json(res, 200, { ok: true, ready, greeting: null, berlinTime: `${p.hour}:${p.minute}` });
  ...
  try { return json(res, 200, await sendGreeting({ mode, base, key, allowDuplicate: true })); }
  catch (error) { console.error('push-daily manual', error); return json(res, 500, { error: error?.message || 'Gruß konnte nicht gesendet werden.' }); }
}
