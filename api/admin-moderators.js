...
export default async function handler(req,res){
  ...
  try{
    ...
    if(userId===adminId && action==="remove")return res.status(400).json({error:"Deinen eigenen Admin-Zugriff kannst du hier nicht entfernen."});

    if(action==="grant"){
      const r=await fetch(`${url}/rest/v1/club_moderators`,{
        method:"POST",headers:{...headers,Prefer:"resolution=merge-duplicates"},
        body:JSON.stringify({user_id:userId,granted_by:adminId})
      });
      ...
      await fetch(`${url}/rest/v1/profiles?id=eq.${encodeURIComponent(userId)}`,{method:"PATCH",headers:{...headers,Prefer:"return=minimal"},body:JSON.stringify({badges:next,updated_at:new Date().toISOString()})});
    }else{
