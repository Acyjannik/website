...
export default async function handler(req, res) {
  ...
  try {
    const allowEmail = smtpConfigured();
    const candidateRes = await rpc(base, key, 'claim_engagement_notification_candidates', {
      p_allow_email: allowEmail
    });
    if (!candidateRes.ok) return json(res, 500, { error: 'Engagement-Kandidaten konnten nicht geladen werden.' });
    ...
    for (const candidate of candidates || []) {
      ...
      const inApp = candidate.in_app_enabled === true;
      const push = candidate.push_enabled === true;
      const email = candidate.email_enabled === true && allowEmail;
      if (!inApp && !push && !email) {
        results.skipped++;
        continue;
      }

      const name = String(candidate.display_name || candidate.username || 'ACY Member').trim();
      const variants = [
      ...
      if (push) {
        try {
          ...
          results.sentPush += Number(result.sent || 0);
          results.failed += Number(result.failed || 0);
        } catch {
          results.failed++;
        }
      }
    ...
    if (notificationRows.length) {
      ...
      if (notificationRes.ok) results.sentInApp += notificationRows.length;
      else results.failed += notificationRows.length;
    }
