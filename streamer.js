(() => {
  ...
  async function roleCheck() {
    ...
    if (!res.ok || !data.ok) throw new Error(data.error || `Rollenprüfung fehlgeschlagen (${res.status}).`);
    return data;
  }
  ...
  async function loadTwitchStatus() {
    ...
    try {
      const r = await fetch('/api/twitch-status', { cache: 'no-store', signal: AbortSignal.timeout(7000) });
      const d = await r.json().catch(() => ({}));
      if (d?.live) {
        set('streamer-status', 'LIVE');
        set('streamer-live-quick', 'LIVE');
        set('streamer-game', d.game || 'Unbekanntes Game');
        set('streamer-game-quick', d.game || 'Unbekanntes Game');
        set('streamer-viewers', `${Number(d.viewerCount || 0)} Zuschauer`);
        set('streamer-live-title', d.title || 'ACYJANNIK ist live');
        set('streamer-live-chip', 'LIVE');
      } else {
        set('streamer-status', 'OFFLINE');
        set('streamer-live-quick', 'Offline');
        set('streamer-game', 'Kein aktuelles Game');
        set('streamer-game-quick', 'Kein aktuelles Game');
        set('streamer-viewers', 'Beim nächsten Stream wieder dabei');
        set('streamer-live-title', 'Aktuell offline');
        set('streamer-live-chip', 'OFFLINE');
      }
      if (status && /Checking|Live-Status wird geladen/i.test(status.textContent || '')) status.textContent = '';
    } catch (e) {
      set('streamer-status', 'OFFLINE');
      set('streamer-live-quick', 'Offline');
      set('streamer-game', 'Nicht verfügbar');
      set('streamer-game-quick', 'Nicht verfügbar');
      set('streamer-viewers', 'Live-Daten momentan nicht verfügbar');
      set('streamer-live-title', 'Twitch-Status konnte nicht geladen werden');
      set('streamer-live-chip', 'UNAVAILABLE');
      if (status) status.textContent = 'Twitch-Livedaten sind momentan nicht verfügbar.';
    }
  }

  function injectStyles() {
    if ($('streamer-v19-rc4-style')) return;
    const style = document.createElement('style');
    style.id = 'streamer-v19-rc4-style';
    style.textContent = `
      ...
      .streamer-tool-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px;margin-top:14px}.streamer-tool-grid .button{min-height:64px;text-align:left}
      .streamer-tool-grid small{display:block;margin-top:3px;opacity:.65;font-size:11px}
  ...
  function init(data) {
    if (!data?.isStreamer && !data?.isAdmin) {
    ...
    injectStyles();
    document.querySelectorAll('#acy-build-marker,.streamer-version,#acy-dev-version-badge').forEach(el => el.remove());
    const badge = document.createElement('div');
    badge.className = 'streamer-version';
    badge.textContent = `${VERSION} · RC`;
    ...
    if (profile.avatar_url) {
      const img = $('streamer-avatar');
      const fallback = $('streamer-avatar-fallback');
      if (img) { img.src = profile.avatar_url; img.hidden = false; }
  ...
  async function start() {
    try {
      ...
      const msg = $('streamer-status-message');
      if (msg) { msg.textContent = error?.message || 'Rolle konnte nicht geprüft werden.'; msg.className = 'club-auth-status error'; }
      set('streamer-live-chip', 'ERROR');
      set('streamer-live-title', 'Streamer-Bereich konnte nicht initialisiert werden');
    }
    $('streamer-logout')?.addEventListener('click', async () => {
