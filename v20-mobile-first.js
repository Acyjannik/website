(() => {
  ...
  // V20 Beta.4: one mobile navigation layer for Club + Pet.
  ...
  const DEFERRED = [
    ['member-spotlight','✨ Spotlight'],['twitch-section','🔴 Live'],['current-game-card','🎮 Aktuelles Game'],
    ['progression-catalog','📈 Fortschritt'],['member-badges-section','🏅 Badges'],['member-events-section','📅 Events'],
    ['club-wheel-section','🎡 Glücksrad'],['daily-streak-section','🔥 Daily-Serie'],['club-quests-section','🎯 Quests'],
    ['club-rewards-section','🎁 Rewards'],['member-news-section','📰 News'],['club-messages','💌 Nachrichten'],
    ['club-chat','💬 Club Chat'],['social-connections-section','👥 Freunde'],['member-directory-section','🧑‍🤝‍🧑 Mitglieder'],
    ['clips-section','🎬 Clips'],['member-leaderboard-section','🏆 Ranking'],['stats-section','📊 Statistik'],
    ['discord-section','🎧 Discord'],['hub-community-games','🎮 Community Games'],['community-poll','🗳️ Community Vote'],
    ['notification-settings','🔔 Benachrichtigungen'],['club-settings-section','⚙️ Einstellungen']
  ];
  ...
  const PET_MORE = CLUB_MORE.map(([group, entries]) => [group, entries.map(([id,text,href]) => [id,text,href || `/club-profile.html#${id}`])]);
  const MORE_GROUPS = IS_PET_PAGE ? PET_MORE : CLUB_MORE;
  ...
  function compactSections(){
    ...
    DEFERRED.forEach(([id]) => {
      ...
      if (isMobile()) {
        if (el.dataset.v20MobileRevealed !== '1') { el.hidden = true; el.dataset.v20MobileDeferred = '1'; }
      } else if (el.dataset.v20MobileDeferred === '1') {
        el.hidden = false; delete el.dataset.v20MobileDeferred;
      }
    });
  }
  ...
  function revealTarget(id){
    ...
    el.hidden = false; el.dataset.v20MobileRevealed = '1'; delete el.dataset.v20MobileDeferred;
    return el;
  }

  function closeMore(){
    const sheet = $('mobile-more-sheet-v181'); const toggle = $('mobile-dock-more-v18');
  ...
  function buildMoreSheet(){
    const sheet = $('mobile-more-sheet-v181'); if (!sheet) return false;
    const grid = sheet.querySelector('.acy-more-grid-v186,.mobile-more-grid-v181'); if (!grid) return false;
    grid.innerHTML = '';
    MORE_GROUPS.forEach(([groupName,entries]) => {
      const label = document.createElement('div'); label.className = 'v20-more-group-label'; label.textContent = groupName; grid.appendChild(label);
      entries.forEach(([id,text,href]) => {
        const item = document.createElement(href ? 'a' : 'button');
        item.className = 'v20-more-item'; item.dataset.v20MoreItem = '1'; item.setAttribute('aria-label',text.replace(/^\S+\s*/,''));
        if (href) { item.href = href; item.dataset.moreHref = href; }
        else { item.type='button'; item.dataset.targetId=id; }
        item.innerHTML = `<span class="v20-more-icon">${text.match(/^\S+/)?.[0] || ''}</span><span class="v20-more-text">${text.replace(/^\S+\s*/,'')}</span>`;
        grid.appendChild(item);
      });
    });
    grid.dataset.v20MoreBuilt = '4';
    return true;
  }
  ...
  function handleV20MoreClick(event){
    const item = event.target.closest('#mobile-more-sheet-v181 [data-v20-more-item]'); if (!item) return;
    event.preventDefault(); event.stopImmediatePropagation();
    if (item.dataset.moreHref) { closeMore(); window.location.assign(item.dataset.moreHref); return; }
    const target = $(item.dataset.targetId); closeMore(); if (target) scrollToTarget(target);
  }

  function handlePetMoreToggle(event){
    if (!IS_PET_PAGE) return;
    const toggle = event.target.closest('#mobile-dock-more-v18'); if (!toggle) return;
    event.preventDefault();
    const sheet = $('mobile-more-sheet-v181'); if (!sheet) return;
    const open = sheet.hidden; sheet.hidden = !open;
    document.body.classList.toggle('mobile-more-open-v181',open);
    toggle.setAttribute('aria-expanded',open?'true':'false');
    toggle.classList.toggle('is-open',open);
    if (open) buildMoreSheet();
  }

  function installStyles(){
    if ($('acy-v20-mobile-first-style')) return;
    const style=document.createElement('style'); style.id='acy-v20-mobile-first-style';
    style.textContent=`
  ...
  function init(){
    labelBetaVersion(); installStyles(); compactSections(); buildMoreSheet();
    if (document.documentElement.dataset.acyV20MoreGuard !== '4') {
      document.documentElement.dataset.acyV20MoreGuard='4';
