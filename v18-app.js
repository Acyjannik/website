(() => {
  ...
  // V19 RC9: club-profile.js expects this legacy upload control even on the
  ...
  async function handleNotificationAction(action,btn){if(btn?.disabled)return;btn&&(btn.disabled=true);try{await notificationAction(action);await window.loadNotifications?.();}catch(e){window.setAcyRefreshStatus?.(e?.message||'Benachrichtigungen konnten nicht aktualisiert werden.','error');}finally{btn&&(btn.disabled=false);}}
