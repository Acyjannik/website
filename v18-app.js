(() => {
  ...
  async function handleNotificationAction(action,btn){if(btn?.disabled)return;btn&&(btn.disabled=true);try{await notificationAction(action);await window.loadNotifications?.();if(action==='clear_all'||action==='mark_all_read')closeNotifications();}catch(e){window.setAcyRefreshStatus?.(e?.message||'Benachrichtigungen konnten nicht aktualisiert werden.','error');}finally{btn&&(btn.disabled=false);}}
  ...
  function init(){
    ...
    addScript('acy-v19-final-runtime-fix','/v19-final-runtime-fix.js?v=19001');
