...
async function ensureDefaultContent() {
  ...
  const defaultGames = [
    ...
    { name: 'MECCHA CHAMELEON', description: 'Hide & Seek · Party · Community', tag: 'VARIETY', image_url: '/assets/games/meccha-chameleon.jpg', featured: false, sort_order: 5 },
...
$('test-ios-push-btn')?.addEventListener('click', async () => {
  ...
  try {
    ...
    if (!response.ok) throw new Error(payload.error || `HTTP ${response.status}`);
    if (status) {
      status.textContent = `iOS-Push gesendet · ${payload.sent || 0} zugestellt · ${payload.failed || 0} Fehler · ${payload.devices || 0} iOS-Gerät(e) gefunden.`;
      status.classList.toggle('error', Number(payload.failed || 0) > 0);
    }
  } catch (error) {
