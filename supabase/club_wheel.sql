...
begin
  ...
  if last_spin is not null and last_spin > now() - interval '24 hours' then
    return jsonb_build_object('ok',false,'cooldown',true,'next_free_at',last_spin + interval '24 hours');
  end if;
  ...
  if roll < 0.30 then
    ...
    reward_key := 'extra_spin'; reward_label := '🎡 Extra Dreh'; reward_value := 0; reward_class := 'spin';
  else
  ...
  return jsonb_build_object(
    ...
    'reward_value',reward_value,'reward_class',reward_class,
    'total_xp',new_xp,'next_free_at',now()+interval '24 hours'
  );
end;
$$;
